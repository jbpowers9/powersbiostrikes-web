"""
PowersBioStrikes Admin Portal - Twitter Post Generator
Create trade alerts, educational content, and track record updates for Twitter/X.
"""
...
try:
    st.set_page_config(page_title="Twitter Generator | PBS Admin", page_icon="🐦", layout="wide")
except:
    pass
...
<p style="color: {COLORS['text_secondary']}; margin-bottom: 30px;">
    Create engaging Twitter content with templates designed for biotech trading.
</p>
...
with st.expander("📝 Twitter Best Practices"):
    st.markdown(f"""
    ### Timing
    - **Best times:** 8-10 AM, 12-1 PM, 5-6 PM (your timezone)
    - **Frequency:** 1-3 tweets per day for growth phase
    ...
    - Quote tweet interesting biotech news
...
st.markdown("<br>", unsafe_allow_html=True)
section_header("Recent Twitter Posts")

posts = load_posts()
twitter_posts = [p for p in posts if p.get('platform') == 'twitter']
recent_twitter = sorted(twitter_posts, key=lambda x: x.get('date', ''), reverse=True)[:5]
...
if recent_twitter:
    for post in recent_twitter:
        ...
        <div style="background: {COLORS['bg_card']}; border: 1px solid {COLORS['steel']};
            ...
            <div style="display: flex; justify-content: space-between; align-items: start;">
                <div style="flex: 1;">
                    <div style="color: {COLORS['text_primary']}; margin-bottom: 5px;">
    ...
    st.info("No Twitter posts yet. Create your first one above!")
